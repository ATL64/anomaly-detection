...
daily_forecast_new<-function(counts,type='history'){ #type can be 'history','monday', or 'weekday'
  
  library(forecast)
  counts<-test[3:92]#to be commented out  
  
  
  a<-Sys.time()
  for(k in 1:nrow(test)){ tryCatch({
    arima_ds<-ts(counts[k,],start=1,freq=7)
